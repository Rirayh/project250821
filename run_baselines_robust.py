...
def analyze_baseline_metrics(metrics_file="baseline_metrics.jsonl"):
...
def run_single_baseline(config, model_type, benchmark):
    ...
    model_args_str = (
        f"base_url=http://127.0.0.1:8001/{endpoint_map[model_type]},"
...
def main():
    ...
    EVAL_CONFIG = {
        ...
        "benchmarks": ["gsm8k", "squad_completion", "drop"], # [MODIFIED] Now a list of benchmarks
        # "limit": 5, # Add limit here for quick testing runs
    }

    metrics_file = "baseline_metrics.jsonl"

    # Start the baseline API server
    server_process = subprocess.Popen(
        ["uvicorn", "baseline_api_server:app", "--host", "0.0.0.0", "--port", "8001"],
    )
    atexit.register(server_process.terminate)
    print(f"Baseline API server started with PID: {server_process.pid}. Waiting...")
    
    health_check_url = "http://127.0.0.1:8001/v1/qwen/chat/completions"
