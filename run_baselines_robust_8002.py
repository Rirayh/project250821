...
def analyze_baseline_metrics(metrics_file="baseline_metrics_8002.jsonl"):
...
def run_single_baseline(config, model_type, benchmark):
    ...
    model_args_str = (
        f"base_url=http://127.0.0.1:8002/{endpoint_map[model_type]},"
    ...
    final_log_path = os.path.join("eval_log_baseline", model_type, "256step", benchmark)
...
def main():
    ...
    metrics_file = "baseline_metrics_8002.jsonl"

    # Start the baseline API server
    server_process = subprocess.Popen(
        ["uvicorn", "baseline_api_server_8002:app", "--host", "0.0.0.0", "--port", "8002"],
    )
    atexit.register(server_process.terminate)
    print(f"Baseline API server started with PID: {server_process.pid}. Waiting...")
    
    health_check_url = "http://127.0.0.1:8002/v1/qwen/chat/completions"
