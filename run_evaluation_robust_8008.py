...
def analyze_metrics(metrics_file="8008"):
...
def run_speculative_evaluation(config, base_log_dir):
    ...
    metrics_file = f"metrics_8008.jsonl"
    ...
    # [MODIFIED] Point PARD_CONFIG_PATH_8008 to the persistent config.json in the log directory.
    server_env["PARD_CONFIG_PATH_8008"] = config_path
    server_process = subprocess.Popen(
        ["uvicorn", "pard_dream_api_server_8008:app", "--host", "0.0.0.0", "--port", "8008"],
        env=server_env,
    )
    ...
    health_check_url = "http://127.0.0.1:8008/v1/chat/completions" 
    ...
    model_args_str = (
        f"base_url=http://127.0.0.1:8008/v1/chat/completions,"
