...
def create_qwen_completion(request: ChatCompletionRequest):
    ...
    with open("baseline_metrics_8002.jsonl", "a") as f:
    ...
    # Log QA record
    model_name_sanitized = BASELINE_CONFIG["qwen_model_name"].replace("/", "__")
    qa_log_filename = f"baselinedebug_8002_{model_name_sanitized}.jsonl"
    qa_record = {"question": prompt, "answer": generated_text, "timestamp": time.time()}
    with open(qa_log_filename, "a") as f:
...
def create_dream_completion(request: ChatCompletionRequest):
    ...
    # Log QA record
    model_name_sanitized = BASELINE_CONFIG["dream_model_name"].replace("/", "__")
    qa_log_filename = f"baselinedebug_8002_{model_name_sanitized}.jsonl"
    qa_record = {"question": prompt, "answer": generated_text, "timestamp": time.time()}
    with open(qa_log_filename, "a") as f:
        f.write(json.dumps(qa_record) + "\n")
