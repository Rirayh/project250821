...
def analyze_metrics(metrics_file="8011"):
...
def run_speculative_evaluation(config, base_log_dir):
    ...
    metrics_file = f"metrics_8011.jsonl"
    ...
    # [MODIFIED] Point PARD_CONFIG_PATH_8011 to the persistent config.json in the log directory.
    server_env["PARD_CONFIG_PATH_8011"] = config_path
    server_process = subprocess.Popen(
        ["uvicorn", "pard_dream_api_server_8011:app", "--host", "0.0.0.0", "--port", "8011"],
        env=server_env,
    )
    ...
    health_check_url = "http://127.0.0.1:8011/v1/chat/completions" 
    ...
    model_args_str = (
        f"base_url=http://127.0.0.1:8011/v1/chat/completions,"
...
if __name__ == "__main__":
    ...
    SWEEP_CONFIG = {
        ...
        "LOGLEVEL" :["DEBUG"],
        # "limit": [10], # Add limit here for quick testing runs
        "log_samples": [True],
    }
