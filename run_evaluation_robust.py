...
def analyze_metrics(metrics_file="metrics.jsonl"):
...
def run_speculative_evaluation(config, base_log_dir):
    ...
    # [MODIFIED] Point PARD_CONFIG_PATH to the persistent config.json in the log directory.
    server_env["PARD_CONFIG_PATH"] = config_path
    server_process = subprocess.Popen(
        ["uvicorn", "pard_dream_api_server:app", "--host", "0.0.0.0", "--port", "8000"],
        env=server_env,
    )
    ...
    health_check_url = "http://127.0.0.1:8000/v1/chat/completions" 
    ...
    model_args_str = (
        f"base_url=http://127.0.0.1:8000/v1/chat/completions,"
...
if __name__ == "__main__":
    ...
    SWEEP_CONFIG = {
        ...
        "benchmark": ["gsm8k", "squad_completion", "drop"],
        "max_gen_toks": [256, 128, 64],
        "max_parallel_draft": [256],
        "draft_temperature": [0.0],
        "draft_steps": [1],
        "acceptance_top_k": [1, 100],
...
#     while time.time() - start_time < timeout:
#         try:
#             # We attempt to send a simple GET request to a known endpoint.
#             # We don't care about the response content, only that a connection can be established.
#             # This indicates the Uvicorn server has started and is listening on the port.
#             requests.get(url, timeout=5)
#             print("\nServer is ready!")
#             return True
#         except requests.exceptions.ConnectionError:
#             # If the connection is refused, the server isn't ready. Wait 2 seconds and retry.
#             print(".", end="", flush=True)
#             time.sleep(2)
#         except requests.exceptions.ReadTimeout:
#             # If the connection is established but times out, we can also consider the server as running.
#             print("\nServer is responsive (timed out), considering it ready.")
#             return True
...
#     total_target_time = 0.0
#     num_requests = 0
#     total_generated_tokens = 0 # NEW: track generated tokens
#     total_wall_time = 0.0      # NEW: track wall-clock time
...
#                 total_draft_time += metric.get("draft_gpu_time_ms", 0)
#                 total_target_time += metric.get("target_gpu_time_ms", 0)
#                 total_generated_tokens += metric.get("generated_tokens", 0) # NEW
#                 total_wall_time += metric.get("time_s", 0) # NEW
#                 num_requests += 1
#             except json.JSONDecodeError:
#                 continue
...
#     acceptance_rate = (total_accepted / total_drafted) if total_drafted > 0 else 0
#     avg_accepted_per_draft = (total_accepted / total_attempts) if total_attempts > 0 else 0
#     # NEW: Calculate overall tokens per second
#     tokens_per_second = (total_generated_tokens / total_wall_time) if total_wall_time > 0 else 0
...
#         "total_draft_gpu_time_ms": total_draft_time,
#         "total_target_gpu_time_ms": total_target_time,
#         "tokens_per_second": tokens_per_second, # NEW
#         "total_wall_time_s": total_wall_time, # NEW
#     }

# def run_speculative_evaluation(config):
#     """
#     Starts a local API server, runs lm-harness against it via the command line,
#     and then reports both standard and custom metrics.
#     """
#     print("--- Starting Speculative Evaluation via Local API ---")
    
#     # MODIFIED: Add timestamp to filenames
#     timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
#     config_path = f"config_pard_{timestamp}.json"

    
#     with open(config_path, 'w') as f:
#         json.dump(config, f)
#     metrics_file = f"metrics.jsonl"
#     results_dir = f"results_pard_{timestamp}"
#     # MODIFIED: Cleanup logic for both file and directory
#     if os.path.exists(metrics_file):
#         os.remove(metrics_file)
...
#     # [MODIFIED SECTION]
#     # Replace time.sleep(60) with the new health check function.
#     health_check_url = "http://127.0.0.1:8000/v1/chat/completions" 
#     if not wait_for_server_ready(health_check_url):
#         print("Script terminating because the server failed to start.")
#         server_process.terminate()
#         server_process.wait()
#         # We use return here instead of exit(1) because this function is called multiple times.
#         return
...
#         "--model_args", model_args_str,
#         "--tasks", config["benchmark"],
#         # "--limit", str(config["limit"]), # REMOVED to run full dataset
#         "--apply_chat_template",
#         # MODIFIED: Output path is now a directory
#         "--output_path", results_dir, 
#         "--batch_size", "1",
#         "--num_fewshot", "0", # Explicitly set zero-shot
#     ]
#     # Add limit back if it exists in the config
#     if "limit" in config:
#         command.extend(["--limit", str(config["limit"])])
...
#     # MODIFIED: Find and read the latest JSON file in the results directory
#     if os.path.isdir(results_dir):
#         # Find all json files recursively
#         list_of_files = glob.glob(os.path.join(results_dir, '**', '*.json'), recursive=True)
#         if list_of_files:
...
#                 except json.JSONDecodeError:
#                     print(f"Error: Could not decode {latest_file}. File might be empty or corrupt.")
#         else:
#             print(f"No result JSON files found in '{results_dir}'.")
#     else:
#         print(f"Results directory '{results_dir}' not found.")
    
#     print("\n--- Custom Speculative Decoding Report ---")
#     custom_metrics = analyze_metrics(metrics_file)
#     if custom_metrics and custom_metrics['num_requests'] > 0:
#         print(f"Total Requests Processed: {custom_metrics['num_requests']}")
#         print(f"Total Wall-Clock Time: {custom_metrics['total_wall_time_s']:.2f} s")
#         print(f"Overall Throughput: {custom_metrics['tokens_per_second']:.2f} Tokens/s") # NEW
#         print("-" * 20)
#         print("Acceptance Metrics:")
#         print(f"  - Acceptance Rate: {custom_metrics['acceptance_rate']:.2%}")
#         print(f"  - Avg. Accepted Tokens per Draft: {custom_metrics['avg_accepted_per_draft']:.2f}")
#         print(f"  - Total Accepted/Drafted Tokens: {custom_metrics['total_accepted_tokens']} / {custom_metrics['total_drafted_tokens']}")
#         print(f"  - Total Draft Attempts: {custom_metrics['total_draft_attempts']}")
#         print("-" * 20)
#         print("GPU Timing Metrics:")
#         print(f"  - Total Draft Model GPU Time: {custom_metrics['total_draft_gpu_time_ms']:.2f} ms")
#         print(f"  - Total Target Model GPU Time: {custom_metrics['total_target_gpu_time_ms']:.2f} ms")
#     else:
#         print("No custom metrics were logged.")
...
#         "acceptance_top_k": 1,
#         "acceptance_threshold": 0.05,
#         # "limit": 5, # Comment out or remove this line to run the full dataset
#     }
